...
const Ceremony = () => {
    ...
    /*
    useLayoutEffect(() => {
        axios.put(`http://localhost:3001/invitations/${person._id}`,{
            "attendance": load
        })
    }, [load, person._id])
    */
    ...
    const updateData = async(e,value) => {  
        ...
        /*
        if(value){
            setImageTrue("img/task_done.svg")
        }else{
            setImageFalse("img/task_done.svg")
        }
        */
        if(value ===  undefined){
    ...
    return (
        <main className={`${CeremonyStyle.main} ${confirmation ? "": CeremonyStyle.disaper }`}>
            ...
            <h2 className={CeremonyStyle.title2}>HAENDA SAN PLO</h2>
            <p className={CeremonyStyle.location}><span ><img src="./img/location_on.svg" alt="Map of location" className={CeremonyStyle.imglocation}/></span> Km. 10, Vía, Subachoque, Cundinamarca</p>
            ...
            <form onSubmit={updateDataFamily} action="#" className={`${CeremonyStyle.formToggle} ${blur ? CeremonyStyle.active: ""}`}>
                ...
                : <>
                 ...
                 <div className={CeremonyStyle.oneInvition}>
                    ...
                    <h4>¡Si, confirmo!</h4>
                <button value={true} onClick={(e) => updateData(e,true)}><img src={`${imageTrue}`} alt="" /></button>
                    </div>
                    <div>
                ...
                {/*<label className={CeremonyStyle.title} htmlFor="confirm">¿Asistes a nuestra boda?</label>
                {person.typeInvitation && <p>Confirma si asistirás seleccionando y dando clic en el botón de enviar.</p>}
                <section>
                {person.typeInvitation ?  person.members.names.map((element, index ) => {
                    return  <>
                ...
                {person.typeInvitation && <button type="submit" className={CeremonyStyle.btnSend}>Enviar</button>}
                <img src="/img/CM_Fecha.png" alt="" className={CeremonyStyle.imgDate} />*/}
            </form>
